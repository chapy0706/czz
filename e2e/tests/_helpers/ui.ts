...
export async function closeAnyOverlay(page: Page) {
  ...
  await page.evaluate(() => {
    const nodes = Array.from(document.querySelectorAll<HTMLElement>('div.fixed.inset-0.z-50'));
...
export async function openEditorFromIndex(page: Page, index = 0): Promise<Locator> {
...
export async function clickSaveIfExists(container: Locator, page: Page) {
  const save = container.getByRole("button", { name: /save|保存|ok|適用/i }).first();
  if (await save.count()) {

...
import { addCommandByType, clickByTestIdRobust, closeAnyOverlay, ensureAtLeastOneCommand } from "./_helpers/ui";
 ...
 */
test("command builder: edit/remove controls work (stable contract)", async ({ page }) => {
  ...
  // 件数が減ることを待つ
  await expect.poll(async () => await rows.count(), { timeout: 10_000 }).toBe(before - 1);
});
